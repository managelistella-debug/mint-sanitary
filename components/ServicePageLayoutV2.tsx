...
interface ServicePageLayoutProps {
  ...
  /** Optional content rendered between the FAQ accordion and the closing CTA. */
  afterFaq?: React.ReactNode;
}

/**
 * EXPERIMENTAL fork of ServicePageLayout — used to prototype design changes
 * on a single page (currently /services/house-cleaning) before rolling them
 * into the shared ServicePageLayout used by every other service page.
 */
export default function ServicePageLayoutV2({
  title,
  heroImage,
  heroIntro,
  heroSubtitle,
  ...
  return (
    <>
      ...
      <Navbar />
      <main>
        {/* ── Hero — its BOTTOM edge is cut into the site's wave curve
              via clip-path, so the Featured Clients section behind it
              shows through the cut. That makes the wave a hole in the
              hero rather than a shape laid on top, so there is no seam:
              everything below the curve is one continuous background.

              Implementation note: this engine only paints clip-path
              polygons whose coordinates are ALL percentages. Pixel or
              calc()/var() coordinates parse but silently render flat,
              which is why the wave band is expressed as a percentage of
              the hero's height (10%) rather than a fixed pixel height.
              ──────────────────────────────────────────────────────── */}
        <style>{`
          .${waveClipClass} {
        ...
        <section className={`${waveClipClass} relative z-10 min-h-[70vh] overflow-hidden`}>
          ...
          {/* Wave removed from here — it's now the top edge of the Featured
              Clients section below, pulled up to overlay the hero image. */}
        </section>

        {/* ── Featured Clients — sits BEHIND the hero (z-0) and is pulled
              up underneath it, so it fills the wave-shaped cut in the
              hero's bottom edge above. The generous top padding keeps the
              heading/logos clear of the deepest point of that cut. ── */}
        <section
        ...
        <section className="relative z-10 bg-white px-4 py-16 sm:px-8 md:px-[60px]">
          <div className="mx-auto max-w-[820px] text-center">
            ...
            <p className="mx-auto mt-3 max-w-[500px] font-body text-[15px] leading-[1.7] text-white/80">
              Use code{" "}
              <strong className="text-white">MINT26</strong> for 10% off
              your first service.{" "}
              <a
                href="/rates/"
